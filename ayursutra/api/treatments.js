...
export default function handler(req, res) {
  ...
  // GET /api/treatments → list all treatments with doctors
  if (!query.id && !query.type && !query.subroute) {
    const enriched = mockTreatments.map(t => ({
      ...t,
      doctors: treatmentDoctors[t.type] || []
    }));
    return res.status(200).json({ success: true, treatments: enriched });
  }

  // GET /api/treatments/doctors → all doctors
  if (query.subroute === "doctors" && !query.type && !query.id) {
    return res.status(200).json({ success: true, data: treatmentDoctors });
  }

  // GET /api/treatments/type/:type/doctors
  if (query.subroute === "type" && query.type && query.subsubroute === "doctors") {
    const doctors = treatmentDoctors[query.type] || [];
    if (doctors.length === 0) {
      return res.status(404).json({ success: false, message: "No doctors found for this treatment type" });
    }
    return res.status(200).json({ success: true, type: query.type, doctors });
  }

  // GET /api/treatments/:id → treatment by id
  if (query.id && !query.subroute) {
    const treatment = mockTreatments.find(t => t.id === parseInt(query.id));
    if (!treatment) {
      return res.status(404).json({ success: false, message: "Treatment not found" });
    }
    return res.status(200).json({
      success: true,
      treatment: { ...treatment, doctors: treatmentDoctors[treatment.type] || [] }
    });
  }
